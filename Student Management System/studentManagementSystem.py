...
class StudentManagementSystem:
    def __init__(self, root):
        ...
        tk.Label(fees_frame, text="Student Fees Information", font=("Arial", 14, "bold"),
                 fg="red").grid(row=0, column=0, columnspan=9, sticky='ew')
        ...
        btn_frame_fees.grid(row=3, column=0, columnspan=9, pady=10)
    ...
    def search_personal(self):
        ...
        for field in self.personal_fields:
            entry = self.personal_entries[field].get().strip()
            if entry:
                db_field = self.get_personal_db_field(field)
    ...
    def search_fees(self):
        conditions = []
        params = []
        for field in self.fees_fields:
            entry = self.fees_entries[field].get().strip()
            if entry:
                db_field = self.get_fees_db_field(field)
                conditions.append(f"{db_field} LIKE %s")
                params.append(f"%{entry}%")
    ...
    # Helper Methods
    def get_personal_db_field(self, field):
        mapping = {
            "Student ID": "student_id",
            "Name": "student_name",
            "Branch": "student_branch",
            "Year": "student_year",
            "BPUT Reg No": "bput_reg_no",
    ...
    def get_fees_db_field(self, field):
        mapping = {
            "Student ID": "student_id",
            "Name": "student_name",
            "Branch": "student_branch",
